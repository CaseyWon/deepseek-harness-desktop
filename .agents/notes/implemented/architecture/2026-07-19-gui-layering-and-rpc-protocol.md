...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` initially reuses the Web application over a managed loopback carrier; a native IPC fetch carrier remains the target transport ([desktop shell decision](../feature/2026-08-14-electron-desktop-shell.md)).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the initial loopback Electron shell; zero workspace dependencies (the registry arrives by structural injection); a native IPC Electron carrier will replace it rather than alter it |
...
The shipped applications preserve the division: the Web application mounts Host, carrier, and browser composition; the desktop application manages that Web profile as a loopback child process; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (future native desktop carrier; the current shell uses `WebApiClient` over loopback) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
