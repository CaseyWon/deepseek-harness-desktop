...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 首先通过受管理的回环载体复用 Web 应用；原生 IPC fetch 载体仍是目标传输方式（见[桌面端外壳决策](../feature/2026-08-14-electron-desktop-shell.md)）。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 用于浏览器访问和首版回环 Electron 外壳；零 workspace 依赖（注册表经结构注入到达）；原生 IPC Electron 载体会替换它而非修改它 |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web，vite 应用）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发 + 每个应用一个拼装模块（web.ts / headless.ts）；vite 应用是 `dsh-client-web` 壳表面之上的薄 main；桌面应用负责 Electron 生命周期与便携打包 | 各应用使用动态 import，因此不会互相加载；dist 定位等 workspace 知识留在 app |
...
已交付的应用保持这一区分：Web 应用挂载 Host、载体与浏览器组合；桌面应用把该 Web profile 作为受管理的回环子进程运行；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| IPC 桥子类（未来的原生桌面载体；当前外壳经回环使用 `WebApiClient`） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
