...
- 绑定测试固定 NUL 结尾的 `char16_t` 解码，并确保不使用 external buffer view。通过打包可执行文件完成一次选择仍是手动 Windows 检查，并推迟到 Windows CI 路线图。
